...
if [[ -n ${SCRIPT} || -n ${CACHE_CLEAR} ]]; then
    ...
    if [[ -n ${SCRIPT} ]]; then
      ssh -v -p 22 -i ${WPE_SSHG_KEY_PRIVATE_PATH} -o StrictHostKeyChecking=no -o ControlPath="$SSH_PATH/ctl/%C" $WPE_FULL_HOST "test -s sites/${WPE_ENV_NAME}/${INPUT_SCRIPT}" > /dev/null 2>&1
