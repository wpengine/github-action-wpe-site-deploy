...
#--exclude="$WPE_MU_PLUGINS"
rsync --rsh="ssh -v -p 22 -i ${WPE_SSHG_KEY_PRIVATE_PATH} -o StrictHostKeyChecking=no" -avm --delete-after --out-format="%n"  --exclude=".*" --exclude="_wpeprivate","wp-config.php" $SRC_PATH "$WPE_DESTINATION"
