...
rsync --rsh="ssh -v -p 22 -i ${WPE_SSHG_KEY_PRIVATE_PATH} -o StrictHostKeyChecking=no" -av --delete-after --out-format="%n"  --exclude=".*" --exclude="$WPE_MU_PLUGINS" $SRC_PATH "$WPE_DESTINATION"
